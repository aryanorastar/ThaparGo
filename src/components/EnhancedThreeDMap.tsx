...
interface EnhancedThreeDMapProps {
...
const LocationMarker = ({ position, color = "#FF5252" }: { position: [number, number, number], color?: string }) => {
    ...
    >
      ...
      >
        ...
        <meshStandardMaterial 
          ...
          roughness={0.7}
          metalness={0.1}
          emissive={isSelected ? new THREE.Color("#FFFF00") : hovered ? new THREE.Color(building.color).lerp(new THREE.Color("#FFFFFF"), 0.3) : new THREE.Color("#000000")}
...
const Scene = ({ onBuildingClick, selectedBuildingId }: EnhancedThreeDMapProps) => {
...
// Main component
const EnhancedThreeDMap: React.FC<EnhancedThreeDMapProps> = ({ onBuildingClick, selectedBuildingId }) => {
  return (
    <div className="relative w-full h-[600px] rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="absolute bottom-4 right-4 bg-white/80 backdrop-blur-sm p-3 rounded-lg shadow-md">
        <div className="text-xs text-gray-700 mb-2">
          ...
          <p>• Scroll: Zoom</p>
          <p>• Click on buildings for details</p>
        </div>
      </div>
    </div>
  );
